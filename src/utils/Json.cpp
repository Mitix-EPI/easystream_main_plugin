...
bool es::utils::Json::JsonArrayIsValidObsArray(json j)
{
	for (auto it : j) {
		if (!it.is_object())
			return false;
...
obs_data_t *es::utils::Json::JsonToObsData(json j)
{
...
void set_json_object(json *j, const char *name, obs_data_item_t *item, bool includeDefault)
{
	...
	j->emplace(name, es::utils::Json::ObsDataToJson(obj, includeDefault));
...
void set_json_array(json *j, const char *name, obs_data_item_t *item, bool includeDefault)
{
	...
	for (size_t idx = 0; idx < count; idx++) {
		...
		json jItem = es::utils::Json::ObsDataToJson(subItem, includeDefault);
...
json es::utils::Json::ObsDataToJson(obs_data_t *d, bool includeDefault)
{
	json j = json::object();
