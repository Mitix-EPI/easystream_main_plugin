...
QStringList es::utils::window::dataHelper::GetStates(Window window)
{
	QStringList states;
...
std::vector<Window> es::utils::window::listHelper::GetTopLevelWindows()
{
...
std::string es::utils::window::stringHelper::GetWindowTitle(size_t i)
{
	Window w = es::utils::window::listHelper::GetTopLevelWindows().at(i);
...
std::vector<std::string> es::utils::window::listHelper::GetWindowList()
{
    std::vector<std::string> windows;
	windows.resize(0);

	for (size_t i = 0; i < es::utils::window::listHelper::GetTopLevelWindows().size(); ++i) {
		if (es::utils::window::stringHelper::GetWindowTitle(i) != "")
			windows.emplace_back(es::utils::window::stringHelper::GetWindowTitle(i));
	}
    return windows;
}

std::string es::utils::window::stringHelper::GetCurrentWindowTitle()
{
    std::string title;
...
std::pair<int, int> es::utils::window::numberHelper::GetCursorPos()
{
...
bool es::utils::window::dataHelper::IsMaximized(const std::string &title)
{
	if (!ewmhIsSupported())
		return false;

	// Find switch in top level windows
	std::vector<Window> windows = es::utils::window::listHelper::GetTopLevelWindows();
...
bool es::utils::window::dataHelper::IsFullscreen(const std::string &title)
{
	if (!ewmhIsSupported())
		return false;
...
bool es::utils::window::dataHelper::IsInFocus(const QString &executable)
{
